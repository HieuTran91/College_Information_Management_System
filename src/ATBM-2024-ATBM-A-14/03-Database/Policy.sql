...
as
    ...
    HK INT;
    NAM INT;
    CURSOR CUR IS (select MAHP, HK, NAM from AD.DANGKY WHERE FUNC_DATE(HK, NAM) > (SYSDATE - 15) AND FUNC_DATE(HK, NAM) < SYSDATE);
begin
    is_dba := SYS_CONTEXT('USERENV', 'ISDBA');
    IF is_dba = 'TRUE' THEN
        RETURN '1=1';
...
as
    ...
    NAM CHAR(4);
    MACT VARCHAR2(4);
    CURSOR CUR IS (select MAHP, HK, NAM, MACT from AD.KHMO WHERE FUNC_DATE(HK, NAM) > (SYSDATE - 15) AND FUNC_DATE(HK, NAM) < SYSDATE);
begin
    is_dba := SYS_CONTEXT('USERENV', 'ISDBA');
    IF is_dba = 'TRUE' THEN
        RETURN '';
